...
(function (window) {
  ...
  Day.prototype._layout = function () {
    ...
    this.cube.innerHTML =
      '<div class="cube__side cube__side--back"></div><div class="cube__side cube__side--left"></div><div class="cube__side cube__side--right"></div><div class="cube__side cube__side--bottom"></div><div class="cube__side cube__side--top"></div><div class="cube__side cube__side--front"><div class="cube__number">' +
      (this.number + 1) +
      "</div></div>";
  ...
  Day.prototype._getDirection = function (ev) {
    var obj = this.cube.querySelector(".cube__side--front"),
      ...
      x =
        ev.pageX -
        (bcr.left + window.pageXOffset) -
        (w / 2) * (w > h ? h / w : 1),
      y =
        ev.pageY -
        (bcr.top + window.pageYOffset) -
        (h / 2) * (h > w ? w / h : 1),
  ...
  Calendar.prototype._initDayEvents = function (day) {
    ...
    instance.cube
      .querySelector(".cube__side--front")
      .addEventListener("mouseenter", instance.mouseenterFn);
  ...
  Calendar.prototype._rotateCalendar = function (mousepos) {
    ...
    var movement = { rx: 3, ry: 3 },
      rotX =
        ((2 * movement.rx) / this.cubes.offsetHeight) * mousepos.y -
        movement.rx,
      rotY =
        ((2 * movement.ry) / this.cubes.offsetWidth) * mousepos.x - movement.ry;

    this.cubes.style.WebkitTransform = this.cubes.style.transform =
  ...
  Snow.prototype._reset = function (flake) {
    ...
    flake.y = 0;
    flake.size = Math.random() * 3.5 + 0.5;
    (flake.speed = flake.size * 0.5), (flake.velY = flake.speed);
    flake.velX = 0;
  ...
  function addBoxes() {
    var a = new Date();
    var activeDays = a.getDate();
    let calendar = document.getElementById("calendar");

    for (let active = 1; active <= activeDays; active++) {
      var activeDiv = document.createElement("div");
      activeDiv.className = "cube";
      activeDiv.setAttribute("data-bg-color", "#1c2d3f");
      activeDiv.setAttribute("data-title", ":)");
      calendar.appendChild(activeDiv);
    }

    for (let inactive = 1; inactive <= 24 - activeDays; inactive++) {
      var inactiveDiv = document.createElement("div");
      inactiveDiv.className = "cube";
      inactiveDiv.setAttribute("data-bg-color", "#6161616");
      inactiveDiv.setAttribute("data-title", "Abwarten, Isabel :)");
      inactiveDiv.setAttribute("data-inactive", "");
      calendar.appendChild(inactiveDiv);
    }
